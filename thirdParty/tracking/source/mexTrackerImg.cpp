...
void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[]) {
  ...
  for (int t = 0; t < mSequenceLength-1; t++) {
    ...
    sprintf(buffer,"forward_%02d.flo",mStartFrame+t+1);
    std::string aName1 = mResultDir+buffer;
    sprintf(buffer,"backward_%02d.flo",mStartFrame+t+1);
    ...
    sprintf(buffer,"tracking%02d.ppm",mStartFrame+t+1);
